...
/* C1 — phoneme feature matrix. Modelled on the Amiga narrator's `featequ.i`.
 * Every phoneme carries a bitmask describing its acoustic / articulatory class;
 * predicates and (future) phonological rules query this mask instead of
 * enumerating phoneme ids. The set of bits is intentionally minimal — only
 * features actually queried by the synthesis code today, plus the missing pieces
 * a phonological-rule pass (C2) is going to need. */
...
typedef struct phoneme_def_t {
    ...
    int is_vowel;
    int voiced;
    say_features_t features;  /* C1 — bit mask of acoustic/articulatory features */
...
/* Composite feature shorthands for the table below. */
...
static int say_phone_has_features(phoneme_id_t id, say_features_t mask)
{
...
{
    return say_phone_has_features(id, F_VOWEL);
}

static int say_is_glide_phone(phoneme_id_t id)
{
    return say_phone_has_features(id, F_GLIDE);
}
...
{
    return say_phone_has_features(id, F_FRIC | F_VOICED);
}

static int say_is_voiced_plosive_phone(phoneme_id_t id)
{
    return say_phone_has_features(id, F_PLOS | F_VOICED);
}
